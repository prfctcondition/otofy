<div align="center">
  ...
  <p>
    ...
    <small>Engineered with Japanese aesthetic minimalism, a studio-grade dual audio engine, and zero compromises.</small>
  </p>
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/Electron-36-4B32C3?style=flat-square&logo=electron&logoColor=white" alt="Electron" />
  ...
  <p>
    <a href="https://github.com/prfctcondition/otofy/releases/latest">
      <img src="https://img.shields.io/badge/Download_Otofy_for_Windows-18181B?style=for-the-badge&logo=windows&logoColor=white" alt="Download Windows Installer" height="38" />
    </a>
  </p>
...
- **No Sign-Up Required**: Launch Otofy and play music immediately. No emails, no passwords, no phone numbers, and *zero behavioral profiling*.
...
### ▸ 02. High-Speed Batch Playlist Downloader
- **One-Click Playlist Downloads**: Save entire albums or `200+` track playlists directly to your hard drive with a single click.
- **Integrated FFmpeg Transcoding Engine**: Bundled high-performance `FFmpeg` audio engine converts streams into pristine audio files with *zero external software needed*.
- **Full ID3 Tag & Cover Art Embedding**: Downloaded tracks automatically include embedded high-resolution (`500×500`) artwork, artist, title, album, and track indices.
...
### ▸ 04. Bidirectional Cloud Playlist Synchronization
...
### ▸ 05. Boundless Dual-Engine Catalog & Streaming Pipeline
...
### ▸ 06. Studio-Grade 10-Band Graphic Equalizer
- **10-Band Precision Filter Bank**: While mainstream desktop clients like Spotify restrict audio tuning to only 6 fixed sliders, Otofy provides a hardware-accelerated 10-band equalizer (`32 Hz` to `16 kHz`) powered by Web Audio API biquad filters.
- **One-Click Presets**: Switch instantly between *Bass Boost*, *Vocal Clarity*, *Electronic*, *Rock*, *Acoustic*, and *Flat*.
- **Real-Time Soundstage**: Fine-tune your frequency curve from `-12 dB` to `+12 dB` with centered precision sliders and zero playback latency.
...
- **Universal Click-to-Seek Karaoke**: Click on any lyric line or phrase to instantly jump to that exact millisecond in the song. In Otofy, lyrics feature universal sub-second synchronized timestamps with instant click navigation.
...
### ▸ 09. Power Library & Local Audio Discovery
...
- **Multi-Select Workflow**: Select multiple tracks at once (<kbd>Ctrl</kbd> / <kbd>Cmd</kbd> + <kbd>Click</kbd>, <kbd>Shift</kbd> + <kbd>Click</kbd>) to bulk-add to playlists, queue next, or download in batch.
...
| **Offline Playback & Tags** | **✔️ Yes *(Embedded ID3 & Art)*** | ⚠️ Encrypted Cache | ⚠️ DRM Encrypted | ⚠️ Encrypted Cache | ⚠️ Mobile Cache Only | ⚠️ If tagged manually |
| **Link-Free Playlist Sharing** | **✔️ Portable Codes** | ❌ Web Links Only | ❌ Web Links Only | ❌ Web Links Only | ❌ Web Links Only | ⚠️ Manual File Copy |
...
| **Graphic Equalizer** | **✔️ 10-Band Parametric** | ⚠️ Limited *(6-Band on PC)* | ⚠️ Presets Only *(No Sliders)* | ❌ No Desktop EQ | ❌ No EQ | ⚠️ Varies *(5-Band)* |
| **Interactive Synced Lyrics** | **✔️ Universal Click-to-Seek** | ⚠️ Rare *(Author-dependent)* | ✔️ Syllable-synced *(Paid)* | ⚠️ Static text only | ❌ No lyrics | ⚠️ Rare / Manual `.lrc` |
...
- ✔ **Proprietary Streaming Bypass Protocol**: Private resilient streaming pipeline with anti-bot protection and zero interruptions.
- ✔ **Batch Playlist Downloader**: Concurrent FFmpeg engine with ID3 & 500x500 artwork embedding.
- ✔ **Link-Free Playlist Sharing**: Export and import playlists via portable Otofy JSON share codes without web links or external hosting.
- ✔ **10-Band Graphic Equalizer**: Web Audio hardware filter bank with audio presets (up from standard 6-band limitations).
- ✔ **Multi-Select & Advanced Sorting**: Bulk library operations, deterministic recents, and tracklist organization.
- ✔ **Local Files Discovery**: Scan and play local `.mp3`, `.flac`, `.opus`, `.m4a`.
...
If Otofy brought harmony to your days, preserved your offline playlists, or simply gave you a peaceful listening experience, support for the craft is received with quiet gratitude:
